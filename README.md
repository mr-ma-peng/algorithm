...
10. [detectCycle(IntListNode head)](src/main/java/org/example/linkedlist/Solution.java#L164)
    ...
    - 实现方式：
        - 使用快慢指针法找到相遇点。
        - 将一个指针重置为链表头部，另一个指针保持在相遇点。
        - 两个指针以相同速度移动，再次相遇时即为环的入口节点。
    - 时间复杂度：O(n)，其中 n 是链表节点数。
    - 空间复杂度：O(1)（仅使用常量级额外空间）。
    - 应用场景：链表环结构分析、内存泄漏检测。
...
## 📁 tree 模块
🔍 文件：Solution.java
1. [maxDepth(TreeNode root)](src/main/java/org/example/tree/Solution.java#L10)
   - 功能：计算二叉树的最大深度。
   - 实现方式：
   ...
   - 应用场景：
     ...
     - 计算树形结构的层级
     - 平衡树判断的基础方法
2. [diameterOfBinaryTree(TreeNode root)](src/main/java/org/example/tree/Solution.java#L30)
   - 功能：计算二叉树的直径。
   - 实现方式：
     - 使用递归方法，分别计算左右子树的最大深度。
     - 当前节点的直径等于左右子树最大深度之和。
   ...
   - 应用场景：
     - 二叉树结构分析
     - 计算树形结构的最长路径
3. [preorderTraversal(TreeNode root)](src/main/java/org/example/tree/Solution.java#L19)
   - 功能：对二叉树进行前序遍历。
   - 实现方式：
